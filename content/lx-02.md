...
donc par exemple si votre disque virtuel fait 50Go maximum, notez quelque chose comme `40G` (pas besoin d'être précis). Ca sera la partition principale. Quand il vous demandez, mettez `[primary]`
...
Là normalement cfdisk n'a encore rien fait pour de vrai, il faut sélectionner `Write` pour taper `yes` et Entreé pour qu'il sauvegarde ça. Vous pouvez quitter cfdisk.
...
Quand ça sera fini, il faudra générer le _fstab_ qui est le descriptif des partitions de votre système (pour que Linux sâche comment les monter au démarrage), pour ça utilisez la commande
...
On dirait que rien à changé, mais `/` est maintenant en réalité `/mnt` , et vous êtes dans votre futur système, avec le shell qu'on vient d'installer, etc...
...
suivant, refaites `Ctrl-W` puis `Entrée` directement). Ca donne par exemple :

![](https://api.epita.litarvan.com/images/lx-02_8.png)

Sauvegardez et quittez avec `Ctrl-X` puis `y` puis Entrée.

Pour appliquer tout ça, lancez la commande `locale-gen` qui devrait générer votre ou vos langue(s) choisie(s).

Maintenant il faut séléctionner la langue par défaut. Pour ça, `nano /etc/locale.conf` et marquez `LANG=fr_FR.UTF-8` (remplacez par `LANG=en_US.UTF-8` si vous préfférez l'anglais), puis sauvegardez
...
drôle, mais de manière générale prenez un truc simple, sans caractères spéciaux ni espace. Editez `nano /etc/hostname` et marquez le simplement dedans (puis sauvegardez et quittez).
...
ni caractère spéciaux ou espace encore une fois. Ensuite lancez la commande `useradd -m -G wheel votreidentifiant` . `-m` permet de creér au passage votre dossier utilisateur, et `-G wheel` permet
...
On va maintenant éditer la configuration sudo pour autoriser les membres du groupe _wheel_ à l'utiliser, pour ça `nano /etc/sudoers` et cherchez cette ligne :
...
Vous devriez arriver sur cet écran de connexion si tout s'est bien passé, j'ai entouré en rouge le bouton pour choisir le bureau sur lequel démarrer :
