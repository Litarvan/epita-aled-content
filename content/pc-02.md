...
pour gérer les fenêtres, ce qui permet de gagner grandement en efficacité. En revanche, il est donc nécéssaire de connaître ces quelques raccourcis primordiaux, pour pouvoir travailler sur les PC de l'école,
...
- `mod+E` Remettre en mode "split" (le mode par défaut), c'est à dire afficher les fenêtres côte à côte. Si le mode "split" est déjà actif, ça interverti entre afficher les fenêtres en colonne ou en ligne
(exemple plus bas)
- `mod+S` Mettre en mode "stack", les fenêtres sont alors les unes derrières les autres, donc on ne voit qu'une seule fenêtre mais elle prend presque tout l'écran
...
déplacé une fenêtre dedans (vous pouvez aussi vous déplacer sur n'importe quelle workspace qu'elles existent ou non). Pas besoin de "créer" les workspaces ni de les supprimer,
c'est automatique quand vous déplacez des fenêtres entre elles.
...
c'est sur cette fenêtre que ça se fera. Il est très important de savoir que **sur i3 la fenêtre "focus" c'est la fenêtre pointée par votre souris**. Si vous ouvrez emacs à gauche et firefox à droite
mais que vous décalez votre souris à droite sur firefox, alors que vous étiez en train d'écrire sur emacs, vous allez vous mettre à écrire sur firefox (donc dans le vide) et non plus sur emacs (certifié
100% vécu).
...
que de saisir votre souris et d'effectuer le déplacement.
...
(Sachant que `gauche` et compagnie font référence aux flèches directionelles). Il est possible pour les connaisseurs d'utiliser les flèches de direction de vim (hjkl pour respectivement gauche bas haut et droite).
...
flèches pour redimensionner petit à petit la fenêtre. Pour quitter le mode resize, appuyez simplement sur `Échap` (pensez-y sinon vous ne pourrez plus faire grand chose).

Ca arrive de rentrer dans ce mode avec une fausse manip, si sans raison vous ne pouvez plus effectuer la plupart des raccourcis i3, n'hésitez pas à appuyer sur `Échap`.
...
ça si vous avez suivi, il faut ouvrir deux fenêtres, ensuite focus la fenêtre de droite, créer un conteneur en "split" vertical avec `mod+V`, puis ouvrir une seconde fenêtre
(du coup dans le conteneur, pour ça il faut rester focus sur le conteneur), ce qui donnera :

![](https://api.epita.litarvan.com/images/pc-02_5.png)

## Configurer i3

Vous pouvez aussi sauter cette étape, mais pour personaliser i3 ou rajouter des raccourcis c'est assez utile, et simple à faire.

La configuration i3 se situe dans `~/.config/i3/config` (c'est un fichier, sous linux il est courant de faire des fichiers sans extensions, c'est à dire sans .truc à la fin du nom),
si vous ne savez pas ce qu'est `~` je vous laisse lire "L'organisation du disque". La configuration est assez simple à comprendre.
...
sachant que la commande pour ça est `systemctl suspend`, il suffit d'ajouter une ligne `bindsym $mod+Shift+x exec systemctl suspend`. A savoir que
la touche mod est référencée par `$mod` et que les lettres représentant les touches du raccourci doivent être en minuscule. Shift lui est représenté par `Shift` (**avec la majuscule**).
...
(Pensez à sauvegarder le fichier de la configuration évidemment). (Note: feh est un programme permettant, entre autre, d'afficher des images notamment en fond d'écran).
