...
un conteneur de fenêtre avec `mod+v`, à ce moment la, la fenêtre focus deviendra un "conteneur". Si vous ouvrez (tout en focusant cette fenêtre, enfin le conteneur)
une nouvelle fenêtre, elle fera aussi partie de ce conteneur. Ce conteneur agira comme une fenêtre au sens où vous pouvez déplacer le conteneur entier, mais aussi
comme une workspace. Vous pouvez par exemple changez la disposition (en appuyant sur `mod+E` par exemple) et ça devrait le faire seulement pour ce conteneur.

Il est possible grâce aux conteneurs par exemple de faire une fenêtre à gauche, et un groupe de fenêtre à droite lui même séparé en deux avec une fenêtre en haut et une fenêtre en bas :
![](https://api.epita.litarvan.com/images/pc-02_6.png)
...
Vous pouvez aussi modifier les raccourcis par défaut, et enlever la barre en bas en commentant (un commentaire commence par `#`) les lignes (normalement tout en bas) à l'intérieur du `bar {...}` (à l'intérieur seulement).
